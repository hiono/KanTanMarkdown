...
(function ($j) {
  ...
  $j.ex.headEach = function (nodes, lv, nums, data, c) {
    ...
    heads.each(function (idx) {
      ...
      if (c.each({
        ...
        headNode: head,
        level: nums
      }, data, c) == false || lv >= c.headTo) return;
      var nAll = head.nextAll(),
          rEnd = false,
          rNode = [];
  ...
  $j.extend($j.ex.TOC.prototype, {
    ...
    _getLabel: function (level, forHead) {
      var o = this,
          c = o.cfg;
      if (!c.numbering || (forHead && !c.numberingHead) || (level.length > c.numberingTo)) return '';
      var label = '';
    ...
    _insContainer: function (grp, headNode, level) {
      ...
      var link = !c.link ? $j(text) : $j(o._getTOCNtext('a', label + text, '', nlen, c.link ? 'href="#' + id + '"' : ''));
